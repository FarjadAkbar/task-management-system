...
export async function POST(req: Request) {
  try {
    ...
    // Ensure employees are provided and construct the participants data
    const participantsData = Array.isArray(body.employees) && body.employees.length > 0
      ? body.employees.map((employeeId: string) => ({
          userId: employeeId,
    ...
    const participants = await Promise.all(
      body.employees.map(async (employeeId: string) => {
